...
class ProcessKibEChunk implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        Http::post('https://n8n.giafn.my.id/webhook/success-import', [
            'status' => 'success',
            'message' => 'Import Chunk KIB E selesai ' . $firstNumberRow . ' sampai ' . $lastNumberRow
        ]);
    }
    ...
    private function storeData($mapped) {
        ...
        if ($mapped['bahan'] == null) {
            $mapped['bahan'] = 'Tidak Diketahui';
        }
        ...
        $detailAsset = [
            ...
            "bahan" => $bahanId,
            "jenis" => null,
            "ukuran" => $mapped['ukuran'],
        ];
    
        DB::pg('asset_detail_lainnya')->insertGetId($detailAsset);
        ...
        if ($mapped['masa_manfaat'] > 0) {
            $this->createPenyusutan($assetId, "Inisiasi Penyusutan Migrasi Asset Lainya", $mapped['tanggal_perolehan']);
        }
    }
    ...
    private function formatData($cells) 
    {
        try {
            return [
                    ...
                    'kelompok_barang' => $cells[17] ?? null,
                    'kode_barang' => $cells[16] ?? null,
                    'jenis_barang' => $cells[17] ?? null,
                    'no_register' => $cells[18] ?? null,
                    'judul' => $cells[19] == "NULL" || $cells[19] == "" ? null : $cells[19],
                    'pencipta' => $cells[20] == "NULL" || $cells[20] == "" ? null : $cells[20],
                    'bahan' => $cells[21] == "NULL" || $cells[21] == "" ? null : $cells[21],
    ...
    private function createPenyusutan($idAsset, $keterangan, $customCreatedAt = null)
    {
        $asset = DB::pg('assets')
            ...
            ->whereIn('jenis_asset_id', [2, 3, 4, 5])
            ->select(
                'assets.*',
