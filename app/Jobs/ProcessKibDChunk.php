...
class ProcessKibDChunk implements ShouldQueue
{
    ...
    private function storeData($mapped) {
        ...
        $asset = [
            ...
            "keterangan_tambahan" => $mapped['keterangan'] . "Lokasi: " . $mapped['details']['Lokasi'],
            "foto" => null,
            "kecamatan" => null,
            "kelurahan_desa" => null,
            "jalan" => null,
        ...
        $detailAsset = [
            "assets_id" => $assetId,
            "masa_manfaat" => $mapped['masa_manfaat'],
            "jenis_perkerasan_barang" => $mapped['details']['Konstruksi'] ?? null,
            "jenis_bahan_struktur_jembatan" => $mapped['details']['Konstruksi'] ?? null,
            "nomor_ruas_jalan" => null,
            "nomor_jaringan_irigasi" => null,
            "titik_kordinat" => null,
            "luas_jalan" => ($mapped['details']['Luas'] ?? null) === "NULL" || $mapped['details']['Luas'] === "" ? null : $mapped['details']['Luas'],
            "panjang" => ($mapped['details']['Panjang'] ?? null) === "NULL" || $mapped['details']['Panjang'] === "" ? null : $mapped['details']['Panjang'],
            "lebar" => ($mapped['details']['Lebar'] ?? null) === "NULL" || $mapped['details']['Lebar'] === "" ? null : $mapped['details']['Lebar'],
        ];

    
        DB::pg('asset_detail_jij')->insertGetId($detailAsset);
    ...
    private function storeKapitalisasi($asset, $details, $data, $upbId, $nilaiSekarang, $masaManfaatSekarang) {
        ...
        $this->updatePenyusutan($asset->id, $data['harga'], $penambahanMasaManfaat, "Rehab/penambahan kapitalisasi Rp. " . number_format($data['harga'], 0, ',', '.'), $data['tanggal_perubahan']);
    ...
    private function formatData($cells) 
    {
        try {
            return [
                    ...
                    'tanggal_pembukuan' => $this->formatDate($cells[25] ?? null),
                    'tanggal_perubahan' => $this->formatDate($cells[26] ?? null),
                    'asal_usul' => $cells[28] ?? null,
                    'kondisi' => $cells[29] ?? null,
                    'masa_manfaat' => $cells[37] ?? null,
                    'harga' => $cells[30] ?? null,
                    'penambahan' => $cells[30] ?? null,
                    'keterangan' => $cells[38] ?? null,
                    'details' => [
                        "Konstruksi" => $cells[19] ?? null,
                        "Panjang" => $cells[20] ?? null,
                        "Lebar" => $cells[21] ?? null,
                        "Luas" => $cells[22] ?? null,
                        "Lokasi" => $cells[23] ?? null,
                    ]
                ];
        } catch (\Exception $e) {
            return null;
